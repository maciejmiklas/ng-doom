...
const parseTitlePic = (bytes: number[], dirs: Directory[], palette: Palette): Either<TitlePic> => {
	...
	const title = find(Directories.TITLEPIC).map(d => tp.parseBitmap(bytes, palette)(d));
	const credit = find(Directories.CREDIT).map(d => tp.parseBitmap(bytes, palette)(d));
	const md = find(Directories.M_DOOM).map(d => tp.parseBitmap(bytes, palette)(d));
	const help = Either.ofEitherArray(find('HELP1').map(d => tp.parseBitmap(bytes, palette)(d)), find('HELP2').map(d => tp.parseBitmap(bytes, palette)(d)));
...
const parseWad = (bytes: number[]): Either<Wad> =>
	dp.parseHeader(bytes)
		...
		.append(w => Either.ofRight(tp.parsePlaypal(bytes, w.dirs)), (w, v) => w.playpal = v) // playpal
